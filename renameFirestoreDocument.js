...
//   3. Optionally provide source and destination document paths via the SRC_PATH
//      and DEST_PATH environment variables or as additional command line
//      arguments:
//         node renameFirestoreDocument.js [serviceAccount.json] [srcPath] [destPath]
//
...
const [, , saArg, srcArgCli, destArgCli] = process.argv;
const serviceAccountPath = process.env.SERVICE_ACCOUNT || saArg;
const srcArg = process.env.SRC_PATH || srcArgCli;
const destArg = process.env.DEST_PATH || destArgCli;
...
const SRC_PATH = srcArg || 'users/sharedUserData';
const DEST_PATH = destArg || 'users/nitin';
...
async function copyDocument(srcRef, destRef) {
  ...
  console.log(`Writing ${destRef.path}`);
