          ...
          return Promise.all(
            urlsToCache.map((urlToCache) => {
              ...
              return cache.add(urlToCache).catch((err) => {
                console.warn(
                  `Service Worker: Non-critical file caching failed for ${urlToCache}`,
                  err,
                );
    ...
    Promise.all([
      ...
      caches.open(CACHE_NAME).then((cache) => {
        return cache.keys().then((requests) => {
          const iconRequests = requests.filter(
            (request) =>
    ...
    event.respondWith(
      ...
      fetch(event.request).catch(() => {
        ...
        return caches.match("/index.html").then((cachedIndex) => {
          if (cachedIndex) {
            console.log(
              "Service Worker: Serving cached index.html for navigation",
            );
            return cachedIndex;
          }
  ...
  if (
    ...
    event.respondWith(
      fetch(event.request).catch((error) => {
        console.log(
          "Network request failed for API/Firebase, no caching attempted:",
          error,
        );
